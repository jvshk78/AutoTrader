...
request_token='Vg9KXB5oZ4QQ8ZdcP63Nk4Wfzs3FmYis'
...
for key in symbols_codes:
    conn.execute("create table if not exists " + key + " (dt datetime,last_price int not null,oi int not null,vol int not null,buy_quantity int not null,sell_quantity int not null,ltq int not null)")

conn.execute("create table if not exists indx (dt_nifty50 datetime,nifty50 int ,dt_banknifty datetime,banknifty int )")
conn.commit()
        ...
        for key in symbols_codes:
            for j in range(len(var.ticks)):
                if (var.ticks[j]['instrument_token'] == symbols_codes[key]['instrument_token']):
                    conn.execute("insert into " + key + " (dt,last_price ,oi ,vol ,buy_quantity ,sell_quantity,ltq) values(datetime('now','localtime'),?,?,?,?,?,?)",
                        (var.ticks[j]['last_price'],var.ticks[j]['oi'],var.ticks[j]['volume'],var.ticks[j]['buy_quantity'],var.ticks[j]['sell_quantity'],var.ticks[j]['last_quantity']))
                if (var.ticks[j]['instrument_token']==256265):
                    conn.execute("insert into indx (dt_nifty50,nifty50) values(datetime('now','localtime'),?) ", (var.ticks[j]['last_price'],))
                if(var.ticks[j]['instrument_token']==260105):
                    conn.execute("insert into indx (dt_banknifty,banknifty) values(datetime('now','localtime'),?)", (var.ticks[j]['last_price'],))
                conn.commit()
